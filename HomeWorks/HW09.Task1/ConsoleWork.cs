    ...
    class ConsoleWork
    {
        internal static bool Chose()
        {
            ...
            while (true)
            {
                ...
                { if (Overrite(key, ConsoleKey.DownArrow, true, 2, -1, out key)) { return true; } }
                if (key.Equals(ConsoleKey.DownArrow))
                { if (Overrite(key, ConsoleKey.UpArrow, false, 1, 1, out key)) { return false; } }
            }
        }

        internal static bool Overrite(ConsoleKey key, ConsoleKey nKey, bool write, byte rowOut, int rowIn, out ConsoleKey pKey)
        {
            pKey = key;
